...
const ProposalModal: React.FC<ProposalModalProps> = ({ isOpen, onClose }) => {
  ...
  const [displayText, setDisplayText] = useState("Will you marry me? 💍");
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setDisplayText("Will you marry me? 💍");
      setExplodingHearts([]);
    }
  }, [isOpen]);

  const moveNoButton = () => {
    // Generate random position within a safe range to keep it visible but hard to click
  ...
  const handleYesClick = () => {
    setYesClicked(true);
    setDisplayText("YES! I LOVE YOU FOREVER! 💖👰‍♀️🤵‍♂️");
      ...
      {isOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          >
            ...
            {!yesClicked ? (
              <div className="flex flex-col md:flex-row items-center justify-center gap-4 md:gap-8 h-32">
                <motion.button
                  whileHover={{ scale: 1.1 }}
                  whileTap={{ scale: 0.95 }}
                  onClick={handleYesClick}
                  className="px-8 py-3 bg-rose-500 hover:bg-rose-600 text-white rounded-full font-bold text-xl shadow-lg shadow-rose-200 transition-all w-40 relative z-10"
                >
                  YES! 💍
                </motion.button>
                ...
                >
                  <button
                    ...
                    className="px-8 py-3 bg-slate-200 text-slate-500 rounded-full font-bold text-lg hover:bg-slate-300 transition-colors w-40 cursor-not-allowed"
                  >
                    No 🏃
                  </button>
                </motion.div>
                ...
                >
                    <p className="text-slate-600">You just made me the happiest person alive! 🎉</p>
                    <button 
                        onClick={onClose}
